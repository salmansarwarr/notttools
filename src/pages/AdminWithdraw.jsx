      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0A151E] via-[#0D1B2A] to-[#0A151E] pt-28 pb-20">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <h1 className="text-3xl font-bold text-white mb-4">{t("wallet_connection_required")}</h1>
            <p className="text-gray-400 mb-8">{t("connect_wallet_fee_withdrawal")}</p>
            ...
            <div className="bg-blue-900/20 border border-blue-600/30 rounded-xl p-6">
              ...
              <p className="text-blue-200 text-sm">{t("connect_wallet_fee_vault")}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 bg-gradient-to-r from-green-600/20 to-blue-600/20 border border-green-500/30 rounded-2xl px-6 py-3 mb-6">
            ...
            <span className="text-green-300 font-semibold">{t("fee_management")}</span>
          </div>

          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            <span className="bg-gradient-to-r from-green-400 via-blue-400 to-cyan-400 bg-clip-text text-transparent">{t("fee_withdrawal")}</span>
          </h1>

          <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">{t("manage_withdraw_fees")}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#192630] to-[#1a2332] border border-gray-700 rounded-2xl p-6 mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-white">{t("connected_wallet")}</h3>
              <p className="text-gray-400">{t("admin_label")} {publicKey?.toBase58()}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-800/50 rounded-xl p-4">
              <p className="text-gray-400 text-sm mb-1">{t("network")}</p>
              <p className="text-white font-semibold">
                {constants.network.type.toUpperCase()}
              </p>
            </div>
            <div className="bg-gray-800/50 rounded-xl p-4">
              <p className="text-gray-400 text-sm mb-1">{t("program_id")}</p>
            ...
            <div className="bg-gray-800/50 rounded-xl p-4">
              <p className="text-gray-400 text-sm mb-1">{t("admin_wallet")}</p>
              <p className="text-white font-semibold text-xs">
          ...
          {publicKey?.toBase58() !== ADMIN_WALLET && (
            <div className="bg-amber-900/20 border border-amber-600/30 rounded-xl p-4 mt-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-amber-200 text-sm"><strong>{t("note_label")}</strong> {t("admin_wallet_withdraw_note")}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="bg-gradient-to-br from-[#192630] to-[#1a2332] border border-gray-700 rounded-2xl p-8 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-2xl font-bold text-white">{t("fee_vault")}</h2>
            </div>
          ...
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400">{t("loading_fee_vault_info")}</p>
            </div>
          ) : feeVaultInfo ? (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-800/50 rounded-xl p-6">
                  <h3 className="text-lg font-semibold text-white mb-2">{t("total_balance")}</h3>
                ...
                <div className="bg-gray-800/50 rounded-xl p-6">
                  <h3 className="text-lg font-semibold text-white mb-2">{t("withdrawable_amount")}</h3>
                  ...
                  <p className="text-gray-500 text-xs mt-2">{t("excluding_rent_exempt")}</p>
                </div>
              </div>

              <div className="bg-gray-800/50 rounded-xl p-6">
                <h3 className="text-lg font-semibold text-white mb-2">{t("fee_vault_address")}</h3>
                ...
                >{t("view_on_explorer_admin")} <ExternalLink className="w-3 h-3" />
                </a>
              </div>
              ...
              <div className="text-center">
                ...
                >
                  {withdrawing ? (
                    <>
                      <RefreshCw className="w-5 h-5 animate-spin" />{t("withdrawing")}</>
                  ) : (
                    <>
                      <Download className="w-5 h-5" />
                      {t("withdraw_all_fees_prefix")} {feeVaultInfo.withdrawable?.toFixed(4) || '0'} SOL)
                    </>
                  )}
                </button>
              ...
              <div className="text-center">
                {feeVaultInfo.balance === 0 && (
                  <p className="text-gray-400 text-sm">{t("no_funds_available")}</p>
                )}

                {publicKey?.toBase58() !== ADMIN_WALLET &&
                  feeVaultInfo.balance > 0 && (
                    <p className="text-amber-400 text-sm mt-2">{t("admin_wallet_only_withdraw")}</p>
                  )}
              </div>
              </div>
            </div>
        ...
        {error && (
          <div className="bg-red-900/20 border border-red-600/30 rounded-xl p-6 mb-8">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-red-300 font-semibold mb-2">{t("error_label")}</h3>
          ...
          <div className="bg-green-900/20 border border-green-600/30 rounded-xl p-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-green-300 font-semibold mb-4">{t("withdrawal_successful")}</h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-green-800/20 rounded-lg p-4">
                    <p className="text-green-200 text-sm mb-1">{t("amount_withdrawn")}</p>
                  ...
                  <div className="bg-green-800/20 rounded-lg p-4">
                    <p className="text-green-200 text-sm mb-1">{t("transaction_label")}</p>
                    <a
